 ...
 */
static void from_ebcdic(
    const unsigned char *buf, size_t nbytes, unsigned char *out)
{
    size_t i;
...
int mume_encoding_check(const unsigned char *buf, size_t nbytes,
                        unsigned int **_ubuf, size_t *_ulen)
{
    int ucs_type;
    int encoding = MUME_ENCODING_UNKNOWN;
    unsigned int *ubuf;
    size_t ulen;
    size_t mlen;

    mlen = (nbytes + 1) * sizeof(ubuf[0]);
    ubuf = calloc_abort(1, mlen);

    if (looks_ascii(buf, nbytes, ubuf, &ulen)) {
        encoding = MUME_ENCODING_ASCII;
    }
    else if (looks_utf8_with_BOM(buf, nbytes, ubuf, &ulen) > 0) {
        encoding = MUME_ENCODING_UTF8_BOM;
    }
    else if (file_looks_utf8(buf, nbytes, ubuf, &ulen) > 1) {
        encoding = MUME_ENCODING_UTF8;
    }
    else if ((ucs_type = looks_ucs16(buf, nbytes, ubuf, &ulen)) != 0) {
        if (ucs_type == 1) {
            encoding = MUME_ENCODING_UTF16_LE;
        }
        else {
            encoding = MUME_ENCODING_UTF16_BE;
        }
    }
    else if (looks_latin1(buf, nbytes, ubuf, &ulen)) {
            encoding = MUME_ENCODING_ISO_8859;
    }
    else if (looks_extended(buf, nbytes, ubuf, &ulen)) {
        encoding = MUME_ENCODING_EXTENDED_ASCII;
    }
    else {
        unsigned char *nbuf = NULL;
        mlen = (nbytes + 1) * sizeof(nbuf[0]);
        nbuf = calloc_abort(1, mlen);

        from_ebcdic(buf, nbytes, nbuf);

        if (looks_ascii(nbuf, nbytes, ubuf, &ulen)) {
            encoding = MUME_ENCODING_EBCDIC;
        }
        else if (looks_latin1(nbuf, nbytes, ubuf, &ulen)) {
            encoding = MUME_ENCODING_INTERNATIONAL_EBCDIC;
        }
